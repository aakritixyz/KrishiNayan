      ...
      <section className="w-full max-w-[420px] rounded-[32px] bg-cream p-6">
        ...
        {mode === "officer" && (
          <>
            ...
            <p className="mt-1 text-sm leading-6 text-muted">For verified government departments and approved organisations. Public registration is disabled.</p>
            ...
            <p className="mt-5 rounded-2xl bg-forest/5 p-3 text-xs leading-5 text-muted">Institutional accounts are provisioned after verification and restricted to their assigned geography.</p>
          </>
        )}
      </section>
    </main>
...
function PasswordInput({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <label className="block text-xs font-semibold text-muted">Password<div className="mt-1 flex items-center gap-2 rounded-2xl border border-forest/15 bg-white px-3 py-3"><Lock size={17} className="text-forest/50" /><input required type="password" value={value} onChange={(e) => onChange(e.target.value)} placeholder="Your password" className="flex-1 bg-transparent text-sm text-forest outline-none placeholder:text-muted" /></div></label>;
}
